...
app.post('/api/generate-room-token', async (req, res) => {
  ...
  try {
    const response = await fetch(`https://${SIGNALING_SERVER}/room-token`, {
      ...
      body: JSON.stringify({ apiKey: APIKEY, secret: CLIENT_SECRET }),
    });
...
app.get('/api/health', (req, res) => {
  res.json({ status: 'ok', configured: !!(APIKEY && CLIENT_SECRET), region: SERVER_REGION });
});

app.listen(PORT, () => {
  console.log(`\n React Room Example server running on http://localhost:${PORT}`);
  console.log(`\n Available endpoints:`);

...
| Package | Purpose |
|---------|---------|
| `@hiyve/react` | Core WebRTC provider and hooks (`useRoomFlow`, `useRecording`, `useParticipants`, `useTranscription`) |
| `@hiyve/react-ui` | `VideoGrid`, `ControlBar`, `Sidebar`, `JoinForm`, `ConnectingScreen`, `WaitForHostScreen`, `DevicePreview` |
    ...
    CloudProvider (AI cloud features via generateCloudToken)
      MoodAnalysisProvider (facial sentiment detection)
        App
          ...
          +-- WaitForHostScreen (SDK) # screen = "waiting-for-host"
...
Both tokens are generated server-side by `@hiyve/admin` middleware so that credentials are never exposed to the browser. `HiyveProvider` and `CloudProvider` auto-discover these endpoints.
...
- `HiyveProvider` handles WebRTC connections, room state, and media devices. Token generation is auto-discovered via `@hiyve/admin` server middleware.
- `CloudProvider` authenticates with the Hiyve Cloud API for AI features (auto-discovers cloud token endpoint)
...
switch (screen) {
  ...
  default:
    return <JoinForm autoConnect storagePrefix="hiyve-ai-example" />;
}
```
